	  ...
	  <button class="btn" id="submit_red" name="submit_red" type="submit">Esqueci Minha Senha</button>
		...
		<?php
			...
			if(isset($_POST['lembrar_senha'])){
				setcookie("lembrar_senha",$_POST['lembrar_senha']);
			}
			if(isset($_POST['submit_login'] ) ){
					loginUsuario();
			}		
		if(isset($_GET['cod_error'])){
			$cod = $_GET['cod_error'];
			if($cod == 40){
				echo "<br><br><center>Usuario Deletado com Sucesso!<br></center>";
			}else if($cod == 50){
				echo "<br><br><center>Faça Login!<br></center>";	
			}
		}
		if(isset($_POST['submit_cad'])){
			header("Location: cadastro.php?".SID);
		}
	?>
</form>
</div>
